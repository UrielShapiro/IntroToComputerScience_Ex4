...
public class Point2D{
	...
	{
		Point2D vec = cen.vector(this);
		_x = vec.x() * ratio + cen.x();
		_y = vec.y() * ratio + cen.y();
	}
	
	public void rotate(Point2D cen, double angleDegrees)
	{
		Point2D _p = new Point2D(_x,_y);
		Point2D vec = cen.vector(_p);
		double r = Math.sqrt(Math.pow(vec.x(), 2) + Math.pow(vec.y(), 2));
		double deg = Math.atan2(vec.y(), vec.x());
		_x = r*Math.cos(deg + angleDegrees) + cen.x();
		_y = r*Math.sin(deg + angleDegrees) + cen.y();
	}
}
